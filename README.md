...
RPT-1.3 fixes the Agent Workspace customer-report action when a consultation record is available but its durable private-report lookup is missing or unavailable. The Workspace now prepares a same-origin report preview before navigation, while normal customer-facing private links keep their existing expiration and deletion behavior. Production runtime remains Cloudflare Pages Functions plus D1; Netlify is not required.
